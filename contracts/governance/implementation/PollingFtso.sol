...
contract PollingFtso is IPollingFtso, AddressUpdatable, GovernedV2 {
    ...
    IIFtsoRewardManager public ftsoRewardManager;
    ...
    function addMember() external override {
        ...
        for (uint256 i = currentRewardEpoch; i > currentRewardEpoch.sub(addAfterRewardedEpochs); i--) {
            uint256 rewardAmount = _getProviderRewards(dataProvider, i - 1);
            if (rewardAmount == 0) {
                revert("no rewards");
            }
        }
    ...
    function removeMember(address _account) external override {
        ...
        if (currentRewardEpoch > memberAddedAtRewardEpoch[_account] + removeAfterNotRewardedEpochs) {
            ...
            for (uint256 i = currentRewardEpoch; i > currentRewardEpoch.sub(removeAfterNotRewardedEpochs); i--) {
                sumRewards += _getProviderRewards(_account, i - 1);
            }
    ...
    function _getProviderRewards(
        ...
        internal view
        returns (uint256 _rewAmount)
    {
        try ftsoRewardManager.getDataProviderPerformanceInfo(_epochId, _account)
        returns (uint256 _rewardAmount, uint256) {
            _rewAmount = _rewardAmount;
        } catch {
            (_rewAmount, ) = ftsoRewardManager.getUnclaimedReward(_epochId, _account);
            if (_rewAmount == 0) {
                try ftsoRewardManager.getStateOfRewards(_account, _epochId) returns (
                    address[] memory _dataProviders,
                    uint256[] memory _rewardAmounts,
                    bool[] memory,
                    bool
                ) {
                    for (uint256 j = 0; j < _dataProviders.length; j++) {
                        if (_dataProviders[j] == _account) {
                            _rewAmount = _rewardAmounts[j];
                            break;
                        }
                    }
                } catch {
