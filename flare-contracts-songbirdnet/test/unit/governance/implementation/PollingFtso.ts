...
contract(`PollingFtso.sol; ${getTestFile(__filename)}; PollingFtso unit tests`, async accounts => {
  ...
  describe("Adding and removing members", async () => {
    ...
    it("Should add member", async () => {
      ...
      const unclaimedReward = web3.utils.sha3("getUnclaimedReward(uint256,address)")!.slice(0, 10); // first 4 bytes is function selector
      const unclaimedRewardReturn = web3.eth.abi.encodeParameters(['uint256', 'uint256'], [100, 10000]);
      ...
      await mockFtsoRewardManager.givenMethodReturn(unclaimedReward, unclaimedRewardReturn);
      await mockFtsoRewardManager.givenMethodRevert(performanceInfo);
    ...
    it("Should add member (unclaimed is zero)", async () => {
      // provider was not removed, was not chilled and was receiving rewards in last epochs

      const unclaimedReward = web3.utils.sha3("getUnclaimedReward(uint256,address)")!.slice(0, 10); // first 4 bytes is function selector
      const unclaimedRewardReturn = web3.eth.abi.encodeParameters(['uint256', 'uint256'], [0, 0]);
      const performanceInfo = web3.utils.sha3("getDataProviderPerformanceInfo(uint256,address)")!.slice(0, 10); // first 4 bytes is function selector

      const stateOfRewards = web3.utils.sha3("getStateOfRewards(address,uint256)")!.slice(0, 10); // first 4 bytes is function selector
      const stateOfRewardsReturn = web3.eth.abi.encodeParameters(['address[]', 'uint256[]', 'bool[]', 'bool'], 
      [[accounts[9], accounts[19]], [100, 99], [true, true], true]);

      await mockFtsoRewardManager.givenMethodReturn(unclaimedReward, unclaimedRewardReturn);
      await mockFtsoRewardManager.givenMethodRevert(performanceInfo);
      await mockFtsoRewardManager.givenMethodReturn(stateOfRewards, stateOfRewardsReturn);
    ...
    it("Should not add member", async () => {
      // provider was not removed, was not chilled but was not receiving rewards in last epochs
      ...
      const stateOfRewards = web3.utils.sha3("getStateOfRewards(address,uint256)")!.slice(0, 10); // first 4 bytes is function selector
      const stateOfRewardsReturn = web3.eth.abi.encodeParameters(['address[]', 'uint256[]', 'bool[]', 'bool'],
      [[accounts[29], accounts[19]], [100, 99], [true, true], true]);
    ...
    it("Should not add member - getStateOfRewards reverts", async () => {
      // provider don't have unclaimed rewards; is delegating explicitly so getStateOfRewards reverts
      ...
      await mockFtsoRewardManager.givenMethodReturn(unclaimedReward, unclaimedRewardReturn);
      await mockFtsoRewardManager.givenMethodRevert(performanceInfo);
      await mockFtsoRewardManager.givenMethodRevert(stateOfRewards);
    ...
    it("Should not remove member that is receiving rewards - unclaimedReward", async () => {
      const unclaimedReward = web3.utils.sha3("getUnclaimedReward(uint256,address)")!.slice(0, 10); // first 4 bytes is function selector
      const unclaimedRewardReturn = web3.eth.abi.encodeParameters(['uint256', 'uint256'], [1, 1]);
      const performanceInfo = web3.utils.sha3("getDataProviderPerformanceInfo(uint256,address)")!.slice(0, 10); // first 4 bytes is function selector
      await mockFtsoRewardManager.givenMethodReturn(unclaimedReward, unclaimedRewardReturn);
      await mockFtsoRewardManager.givenMethodRevert(performanceInfo);
    ...
    it("Should not remove member that is receiving rewards - stateOfRewards", async () => {
      const unclaimedReward = web3.utils.sha3("getUnclaimedReward(uint256,address)")!.slice(0, 10); // first 4 bytes is function selector
      const unclaimedRewardReturn = web3.eth.abi.encodeParameters(['uint256', 'uint256'], [0, 0]);
      const performanceInfo = web3.utils.sha3("getDataProviderPerformanceInfo(uint256,address)")!.slice(0, 10); // first 4 bytes is function selector
      await mockFtsoRewardManager.givenMethodReturn(unclaimedReward, unclaimedRewardReturn);
      ...
      const stateOfRewards = web3.utils.sha3("getStateOfRewards(address,uint256)")!.slice(0, 10); // first 4 bytes is function selector
      // provider delegated to himself and received rewards
      const stateOfRewardsReturn = web3.eth.abi.encodeParameters(['address[]', 'uint256[]', 'bool[]', 'bool'], 
      [[accounts[3], accounts[19]], [100, 99], [true, true], true]);
    ...
    it("Should remove member that is not receiving rewards", async () => {
      const unclaimedReward = web3.utils.sha3("getUnclaimedReward(uint256,address)")!.slice(0, 10); // first 4 bytes is function selector
      const unclaimedRewardReturn = web3.eth.abi.encodeParameters(['uint256', 'uint256'], [0, 0]);
      const performanceInfo = web3.utils.sha3("getDataProviderPerformanceInfo(uint256,address)")!.slice(0, 10); // first 4 bytes is function selector
      await mockFtsoRewardManager.givenMethodReturn(unclaimedReward, unclaimedRewardReturn);
      ...
      // provider didn't delegate to himself
      const stateOfRewardsReturn = web3.eth.abi.encodeParameters(['address[]', 'uint256[]', 'bool[]', 'bool'], 
      [[accounts[23], accounts[13]], [100, 99], [true, true], true]);
      await mockFtsoRewardManager.givenMethodReturn(stateOfRewards, stateOfRewardsReturn);
    ...
    it("Should remove member that is not receiving rewards - getStateOfRewards reverts", async () => {
      const unclaimedReward = web3.utils.sha3("getUnclaimedReward(uint256,address)")!.slice(0, 10); // first 4 bytes is function selector
      const unclaimedRewardReturn = web3.eth.abi.encodeParameters(['uint256', 'uint256'], [0, 0]);
      const performanceInfo = web3.utils.sha3("getDataProviderPerformanceInfo(uint256,address)")!.slice(0, 10); // first 4 bytes is function selector
      await mockFtsoRewardManager.givenMethodReturn(unclaimedReward, unclaimedRewardReturn);
      ...
      // provider didn't delegate to himself
      await mockFtsoRewardManager.givenMethodRevertWithMessage(stateOfRewards, "error");
