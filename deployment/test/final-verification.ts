...
function calculateAuthorizedInflationWei(parameters: ChainParameters, totalAuthorizedDailyInflationWei: BN, contractName: string): BN {
...
contract(`final-verification.ts system tests`, async accounts => {
  ...
  describe(Contracts.INFLATION, async () => {
    it("validate first inflation time slot is initialized with correct inflation percentage", async () => {
      ...
      const calculatedInflationWei = BN(parameters.totalNativeSupplyNAT).sub(BN(parameters.totalExcludedSupplyNAT)).mul(BN(10).pow(BN(18)))
