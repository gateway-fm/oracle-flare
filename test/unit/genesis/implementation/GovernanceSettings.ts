...
contract(`GovernanceSettings.sol; ${getTestFile(__filename)}; GovernanceSettings unit tests`, async accounts => {
    const GOVERNANCE_SETTINGS_ADDRESS = "0x183fAfF6997C98A812A3B98748Fc80241D08f312";
    ...
    it("cannot initialise twice", async () => {
        await expectRevert(governanceSettings.initialise(accounts[11], 3600, [governance, executor]),
            "already initialised");
    });

    it("only deployer can initialise", async () => {
    ...
    it("check timelock size in initialisation", async () => {
        await network.provider.send("hardhat_setStorageAt", [GOVERNANCE_SETTINGS_ADDRESS, "0x0", constants.ZERO_BYTES32]);  // clear initialisation
        await expectRevert(governanceSettings.initialise(accounts[11], "1000000000000000", [governance, executor]),
            "timelock too large");
    });
});
